...
def plot_camera_ratio_avg(shot_no, line_ch, frame_tgt, num_frames, vmin, vmax, avg_time):
    camera_dict_ratio_avg = calc_avg(shot_no, line_ch, frame_tgt, num_frames, avg_time)
    
    dir_out = os.path.join(camera_dict_ratio_avg['result_dir'], 'ratio', str(shot_no) + '_ch' + str(line_ch[0]) + '_over_ch' + str(line_ch[1]) + '_avg_' + str(avg_time))
    os.makedirs(dir_out, exist_ok=True)

#%%
    zaxis = np.linspace(0, len(camera_dict_ratio_avg['data_avg'][0,0,:]), len(camera_dict_ratio_avg['data_avg'][0,0,:])+1)
    xaxis = np.linspace(0, len(camera_dict_ratio_avg['data_avg'][0,:,0]), len(camera_dict_ratio_avg['data_avg'][0,:,0])+1)
    ppmm = 112/(int(camera_dict_ratio_avg["diam_flange"]))  # mm per pixel
    ...
    # max intensity
    # ymax = np.max(camera_dict_ratio_avg['data_avg'])
    # print('ymax = ' + str(ymax))
...
#%% Save fig
    for i, img in tqdm(enumerate(camera_dict_ratio_avg['data_avg']), total=camera_dict_ratio_avg['data_avg'].shape[0], desc='Saving figs...'):
        ...
        # Title
        time_quot = camera_dict_ratio_avg["avg_time_list"][i] # quotient
        time_quot = round(time_quot, 2)
        ax.set_title('#'+str(shot_no)+'_ch' + str(line_ch[0]) + '_over_ch' + str(line_ch[1]) +'\n@' +  str(time_quot) + 'ms-' + str(time_quot+avg_time) + 'ms', fontsize=16)
        ...
        num_frames_per_block = int(avg_time * int(camera_dict_ratio_avg['frame_rate']) / 1000)
        plt.savefig(os.path.join(dir_out, str(shot_no) + '_ch' + str(line_ch[0]) + '_over_ch' + str(line_ch[1]) + '_'  + str(frame_tgt+num_frames_per_block*i) + '-'  + str(frame_tgt+num_frames_per_block*(i+1)-1) + '.svg'))
